...
## Checklist
- [ ] Create blog_posts table in Supabase with necessary columns (title, slug, content, excerpt, featured_image, author_id, category, status, tags, meta_description)
- [ ] Set up RLS policies for blog_posts table (public read for published, authenticated write)
- [ ] Create categories table and enum for post status (draft/published)
- [ ] Create admin dashboard page (/admin) with list of all blog posts
- [ ] Create blog post editor page (/admin/posts/new) with rich text editor and form fields
- [ ] Implement image upload to Supabase Storage for featured images and inline content images
- [ ] Create dynamic blog post detail pages ([slug].tsx)
- [ ] Update homepage ArticleCard component to pull from database instead of mock data
- [ ] Add blog post preview functionality
- [ ] Create blog service for CRUD operations
